...
for member in members:
    ...
    for filename in os.listdir(base_membar_dir):
        ...
        # 閾値
        threshold_img = cv2.threshold(img, 100, 255, cv2.THRESH_TOZERO)[1]
        threshold_file_path = os.path.join(base_membar_dir, basename + '_normal_' + 'threshold' + ext)
        cv2.imwrite(threshold_file_path, threshold_img)
        # Gaussian
        gaussian_img = cv2.GaussianBlur(img, (5, 5), 0)
        gaussian_file_path = os.path.join(base_membar_dir, basename + '_normal_' + 'gaussian' + ext)
